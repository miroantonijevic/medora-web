...
const INPUT: React.CSSProperties = {
  ...
  border: '1px solid #ddd', padding: '13px 14px', fontSize: 15,
  background: '#fff', color: '#333', fontFamily: 'inherit',
...
export function InquiryForm() {
  ...
  return (
    <main style={{ background: '#fffaf5', minHeight: '60vh', padding: 'clamp(40px, 6vh, 80px) clamp(24px, 4vw, 48px) 80px' }}>
      <div style={{ maxWidth: 780, margin: '0 auto' }}>
        <nav style={{ fontSize: 13, color: '#888', marginBottom: 36, display: 'flex', flexWrap: 'wrap' }} aria-label="breadcrumb">
          <Link href="/" style={{ color: '#009bdb', textDecoration: 'none' }}>{tNav('home')}</Link>
          <span style={{ margin: '0 8px', color: '#ccc' }}>/</span>
          <span style={{ color: '#555' }}>{t('title')}</span>
        </nav>

        <h1 style={{ fontSize: 'clamp(28px, 4vw, 42px)', fontWeight: 700, color: '#012B59', marginBottom: 10 }}>
          {t('title')}
        </h1>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
            ...
            <label style={LABEL}>
              ...
              <input name="email" type="email" required value={form.email} onChange={set} style={INPUT} />
            </label>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
            <label style={LABEL}>
            ...
            <label style={LABEL}>
              ...
              <input name="roomPreference" value={form.roomPreference} onChange={set} style={INPUT} />
            </label>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
            <label style={LABEL}>
            ...
            <label style={LABEL}>
              ...
              <input name="departure" type="date" value={form.departure} onChange={set} style={INPUT} />
            </label>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
            <label style={LABEL}>
              <span style={LABEL_TEXT}>{t('adults')}</span>
              <input name="adults" type="number" min="1" max="20" value={form.adults} onChange={set} style={INPUT} />
            </label>
            <label style={LABEL}>
              <span style={LABEL_TEXT}>{t('children')}</span>
              <input name="children" type="number" min="0" max="10" value={form.children} onChange={set} style={INPUT} />
            </label>
          </div>
          <label style={{ ...LABEL, marginBottom: 32 }}>
            <span style={LABEL_TEXT}>{t('message')}</span>
            <textarea
              name="message" rows={5} value={form.message} onChange={set}
              style={{ ...INPUT, resize: 'vertical' }}
            />
          </label>

          <p style={{ fontSize: 13, color: '#888', marginBottom: 24, maxWidth: 560 }}>{t('note')}</p>

          <button
            type="submit"
            style={{
              background: '#009bdb', color: '#fff', border: 'none',
              padding: '16px 44px', fontSize: 13, fontWeight: 700,
