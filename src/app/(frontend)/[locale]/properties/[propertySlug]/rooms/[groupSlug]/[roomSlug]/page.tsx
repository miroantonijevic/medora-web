...
export async function generateStaticParams() {
  ...
  for (const property of properties) {
    ...
    for (const room of rooms) {
      const g = room.group as { slug?: string } | null | undefined
      const groupSlug = typeof g === 'object' && g?.slug ? g.slug : null
      if (!groupSlug) continue
      params.push({ propertySlug: property.slug, groupSlug, roomSlug: room.slug })
    }
  }
  return params
}
...
export default async function RoomDetailPage({ params: paramsPromise }: Args) {
  ...
  const groupDoc = room.group as { name?: unknown } | null | undefined
  const groupName =
    groupDoc && typeof groupDoc === 'object' && groupDoc.name
      ? String(groupDoc.name)
      : tNav('rooms')

  type MediaDoc = { url?: string; alt?: string }
  const images = ((room.images ?? []) as MediaDoc[]).filter((img): img is Required<MediaDoc> =>
    Boolean(img.url),
  )
    ...
    <main>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(24px, 4vw, 48px)' }}>
        ...
        >
          ...
          <span style={{ margin: '0 8px', color: '#ccc' }}>/</span>
          <Link
            href={`/properties/${propertySlug}/rooms/${groupSlug}`}
            style={{ color: '#009bdb', textDecoration: 'none' }}
          >
            {groupName}
          </Link>
      ...
      >
        ...
        >
          <div>
            ...
            {room.bedType && (
              <p style={{ fontSize: 17, color: '#222', margin: '0 0 6px' }}>
                {String(room.bedType)}
              </p>
            )}
          </div>
