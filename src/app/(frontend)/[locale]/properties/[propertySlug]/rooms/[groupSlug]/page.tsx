...
import { getPropertyBySlug, getPublishedProperties, getRoomGroupBySlug, getRoomGroupChildren, getRoomsByGroup } from '@/lib/queries'
...
export async function generateMetadata({ params: paramsPromise }: Args): Promise<Metadata> {
  ...
  if (!property) return {}
  return { title: `${group ? String(group.name) : groupSlug} — ${String(property.name)} | Medora Hotels` }
}
...
export default async function RoomGroupPage({ params: paramsPromise }: Args) {
        ...
        <div style={{ background: '#012B59', padding: '48px clamp(24px, 4vw, 48px) 40px' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            <nav style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', marginBottom: 16, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 4 }}>
              ...
              <span style={{ margin: '0 6px' }}>/</span>
              <Link href={`/properties/${propertySlug}/rooms`} style={{ color: 'rgba(255,255,255,0.7)', textDecoration: 'none' }}>{tNav('rooms')}</Link>
              <span style={{ margin: '0 6px' }}>/</span>
              <span style={{ color: '#fff' }}>{groupName}</span>
            </nav>
            <h1 style={{ color: '#fff', fontSize: 'clamp(28px, 3vw, 42px)', fontWeight: 700, margin: 0 }}>{groupName}</h1>
          </div>
        </div>
                ...
                >
                  <article style={{
                    ...
                    <div style={{ position: 'relative', minHeight: 300, overflow: 'hidden' }}>
                      {heroImage?.url ? (
                        <Image src={heroImage.url} alt={heroImage.alt ?? childName} fill style={{ objectFit: 'cover' }} sizes="480px" />
                      ) : (
                    ...
                    <div style={{ padding: '44px 48px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      <h2 style={{ fontSize: 'clamp(22px, 2.5vw, 32px)', fontWeight: 700, color: '#012B59', margin: '0 0 12px', lineHeight: 1.25 }}>
                        {childName}
                      </h2>
                      {child.description && (
                        <p style={{ fontSize: 14, color: '#666', margin: '0 0 36px' }}>{String(child.description)}</p>
                      )}
                      <span style={{
                        display: 'inline-block',
                        border: '1.5px solid #012B59', color: '#012B59',
                        padding: '12px 30px', fontSize: 12, fontWeight: 700,
  ...
  return (
    <main>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(40px, 5vh, 64px) clamp(24px, 4vw, 48px)' }}>
        {rooms.length === 0 && (
          <p style={{ color: '#666' }}>{tRooms('noRooms')}</p>
        )}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 48 }}>
          {rooms.map((room) => {
            type MediaDoc = { url?: string; alt?: string }
            const images = (room.images ?? []) as MediaDoc[]
            const heroImg = images[0]
            const bookingLink = bookingService.getBookingLink({ propertySlug, roomSlug: room.slug })
            ...
            return (
              <article key={room.id}>
                ...
                >
                  ...
                  <h2 style={{ fontSize: 'clamp(18px, 1.8vw, 22px)', fontWeight: 600, color: '#1a1a1a', margin: '20px 0 8px', lineHeight: 1.35 }}>
                    {String(room.name)}
                  </h2>
                </Link>
                <p style={{ fontSize: 13, color: '#666', margin: '0 0 20px' }}>
                  {[room.size && String(room.size), room.bedType && String(room.bedType), room.capacity && `${room.capacity} ${tRooms('guests').toLowerCase()}`].filter(Boolean).join(' | ')}
                </p>
                <div style={{ display: 'flex', gap: 12 }}>
                  <a
                    href={bookingLink.href}
                    style={{
                      display: 'inline-block', background: '#012B59', color: '#fff',
                  ...
                  <Link
                    href={`/properties/${propertySlug}/rooms/${groupSlug}/${room.slug}`}
                    style={{
                      display: 'inline-block', border: '1.5px solid #333', color: '#333',
                      padding: '10px 24px', fontWeight: 600, fontSize: 13,
                      textDecoration: 'none',
                    }}
                  >
                    {tRooms('select')}
